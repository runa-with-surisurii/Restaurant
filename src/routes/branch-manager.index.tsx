...
type Analytics = { orders:number; revenue:number; todaySales?:number; totalRevenue?:number; customers:number; growth:number; weekly_sales:{day:string;sales:number}[]; recent_menus?: RecentMenu[]; recentMenus?: RecentMenu[]; current_menu_sold?: RecentMenu|null; currentMenuSold?: RecentMenu|null; inventory_usage:{ingredient:string;used:number;unit:string;beforeStock:number;remaining:number;orderId:string;date:string}[]; sales_prediction?: Prediction; salesPrediction?: Prediction };
...
function BranchDashboard(){
  ...
  useEffect(()=>{
    ...
    Promise.all([fetch("http://127.0.0.1:8000/api/branches").then(r=>r.json()), fetch(`http://127.0.0.1:8000/api/branch/dashboard/${branchId}`).then(r=>{if(!r.ok) throw new Error(`Dashboard API ${r.status}`); return r.json();})])
      .then(([branches,data])=>{setBranch(branches.find((b:Branch)=>String(b.branchId ?? b.storeNumber)===String(branchId))); setAnalytics(data);}).catch(console.error).finally(()=>setLoading(false));
  },[adminUser,branchId]);
  ...
  return <div className="min-h-screen bg-muted/30 p-8 space-y-8">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <Card><CardHeader><CardTitle>Recent Menu</CardTitle></CardHeader><CardContent className="space-y-4">{recentMenus.length ? recentMenus.slice(0,2).map((menu,index)=><div key={`${menu.menuId}-${index}`} className="flex items-center justify-between border-b last:border-b-0 pb-4 last:pb-0"><div className="flex items-center gap-4"><img src={menu.image} alt={menu.name} className="h-16 w-16 rounded-xl object-cover" onError={e=>{e.currentTarget.src="/menu/default.png"}}/><div><p className="font-bold">{menu.name}</p><p className="text-sm text-muted-foreground">Recent confirmed order</p></div></div><span className="text-sm text-muted-foreground">Qty {menu.quantity}</span></div>) : <p className="text-sm text-muted-foreground">No menu sold yet</p>}</CardContent></Card>
    </div>
