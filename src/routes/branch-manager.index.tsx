...
function BranchDashboard(){
  ...
  return <div className="min-h-screen bg-muted/30 p-8 space-y-8">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <Card><CardHeader><CardTitle>Recent Menu</CardTitle></CardHeader><CardContent className="space-y-4">{recentMenus.length ? recentMenus.slice(0,2).map((menu,index)=><div key={`${menu.menuId}-${index}`} className="flex items-center justify-between border-b last:border-b-0 pb-4 last:pb-0"><div className="flex items-center gap-4"><img src={menu.image} alt={menu.name} className="h-16 w-16 rounded-xl object-cover" onError={e=>{e.currentTarget.src="/menu/default.png"}}/><div><p className="font-bold">{menu.name}</p><p className="text-sm text-muted-foreground">Recent confirmed order</p></div></div><span className="text-sm text-muted-foreground">Qty {menu.quantity}</span></div>) : <p className="text-sm text-muted-foreground">No menu sold yet</p>}<div className="border-t pt-4"><p className="text-sm text-muted-foreground">Total Revenues</p><p className="text-2xl font-bold mt-1">${(analytics?.totalRevenue || 0).toLocaleString()}</p></div></CardContent></Card>
    </div>
